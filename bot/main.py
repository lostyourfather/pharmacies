...
async def echo_handler(message: types.Message) -> None:
    try:
        header, description, is_prescription, img_src, value, currency = get_data(message.text.lower().strip())
...
def get_data(request_name):
    ...
    with Session(engine) as session:
        ...
        price = prices[0][0]
        return product.header, product.description, product.is_prescription, product.img_src, price.value, price.currency
